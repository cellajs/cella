/**
 * Shared configuration for Artillery load tests. Everything is derived from the
 * fork's own config (`appConfig`, `backend/.env`), so bench follows a fork's port
 * offset automatically. Dev-only tool tied to the local stack.
 */
