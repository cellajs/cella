...
describe('computeBatchUnifiedDeltas', () => {
  it('batch of 5 attachment creates in same org: accumulates deltas', () => {
    ...
    expect(plan.countDeltasByChannelKey.get('org-1')).toEqual({
      'e:c:attachment': 5,
      'e:c:h:attachment': 5,
      'e:li:h:attachment': expect.any(Number),
    });
  });
...
describe('activity stamps (e:li:h:{type} / e:lu:h:{type})', () => {
  ...
  it('two creates in one batch max-merge the stamp (timestamps must not sum)', () => {
    ...
    expect(plan.countDeltasByChannelKey.get('org-1')).toEqual({
      'e:c:attachment': 2,
      'e:c:h:attachment': 2,
      'e:li:h:attachment': Date.parse(laterCreatedAt),
    });
  });
  ...
  it('missing createdAt falls back to Date.now()', () => {
    ...
    const stamp = plan.countDeltasByChannelKey.get('org-1')?.['e:li:h:attachment'];
  ...
  it('a row created directly published stamps li: from publishedAt', () => {
    ...
    expect(plan.countDeltasByChannelKey.get('org-1')).toEqual({
      'e:c:attachment': 1,
      'e:c:h:attachment': 1,
      'e:li:h:attachment': Date.parse(publishedAt),
    });
  });
  ...
  it('update with missing updatedAt falls back to Date.now()', () => {
    ...
    const stamp = plan.countDeltasByChannelKey.get('org-1')?.['e:lu:h:attachment'];
    expect(stamp).toBeGreaterThanOrEqual(before);
    expect(stamp).toBeLessThanOrEqual(after);
  });
  ...
  it('soft-deletes never stamp (remapped to a delete internally)', () => {
    ...
    expect(plan.countDeltasByChannelKey.get('org-1')).toEqual({ 'e:c:attachment': -1, 'e:c:h:attachment': -1 });
  });
