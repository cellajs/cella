...
> This document explains how the worker does that, and which invariants to keep in mind when you change tracked tables or touch the pipeline. For what these outputs mean to clients — notification shapes, catchup, HLC merge — see [Sync engine](/docs/page/architecture/sync-engine).
...
The worker keeps no durable state of its own: everything needed to resume lives in the replication slot's restart position. Whatever was not yet acknowledged is simply redelivered.
...
Acknowledgement is **manual** (`acknowledge: { auto: false }`): the slot only advances after events are fully processed (see delivery semantics below). Standby heartbeats are acknowledged when Postgres requests a reply, so an idle worker doesn't time out.
