...
> [!IMPORTANT] `common.json` and `app.json` are **merged into a single `c` namespace** at runtime (see `frontend/src/lib/i18n-locales.ts`). All keys from _both_ files — including keys you add to `app.json` — are referenced as `t('c:key')`. There is no `app:` or `common:` namespace, so `t('app:key')` resolves to nothing. The backend loads `common.json` under the same `c` namespace.
