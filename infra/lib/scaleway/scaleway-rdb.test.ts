...
describe('createRdbClient', () => {
  it('finds an instance by exact name', async () => {
    ...
    expect(calls[0]).toMatchObject({
      url: 'https://api.scaleway.com/rdb/v1/regions/nl-ams/instances?name=cella-postgres',
    });
  });
    ...
    const { fn } = makeFetch([
      {
        method: 'GET',
        match: '/instances',
        body: { instances: [{ id: 'i-2', name: 'cella-postgres-staging', status: 'ready' }], total_count: 1 },
      },
    ]);
...
describe('waitForBackupReady', () => {
  ...
  it('throws on an errored backup instead of waiting out the timeout', async () => {
    await expect(waitForBackupReady(stub(['error']), 'bk-1', { intervalMs: 0, sleep: async () => {} })).rejects.toThrow(
      /failed/,
    );
  });
