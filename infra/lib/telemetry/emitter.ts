...
export function createTelemetry(opts: TelemetryOptions): Telemetry {
  ...
  let exporter: { endpoint: string; headers?: Record<string, string> } | undefined = opts.endpoint
    ? { endpoint: opts.endpoint, headers: opts.headers }
    : undefined;
  ...
  return {
    ...
    startSpan,
    event(
      nameOrDef: EventDef | string,
      attrs: Record<string, AttrValue> = {},
      eventOpts: { severity?: Severity; body?: string; ctx?: SpanContext } = {},
    ) {
