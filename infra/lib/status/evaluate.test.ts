...
describe('credential degradation', () => {
  it('scaleway-tier checks are unknown (not error) without credentials', () => {
    const report = evaluateStatus(
      base({ credentialsAvailable: false, stateBucketExists: undefined, lock: undefined, rollout: undefined, requiredSecretsMissing: undefined }),
    )
...
describe('nextAction priority (lifecycle order)', () => {
  it('missing pulumi outranks everything', () => {
    const report = evaluateStatus(base({ tooling: { pulumi: false, dockerBuildx: true, gh: true }, stackState: 'fresh', projectId: undefined }))
  ...
  it('a fresh stack points at setup', () => {
    const report = evaluateStatus(base({ stackState: 'fresh', projectId: undefined, stateBucketExists: false, rollout: undefined, requiredSecretsMissing: undefined, lock: undefined }))
...
describe('live service checks', () => {
  ...
  it('serving a stale sha warns and points at deploy', () => {
    const report = evaluateStatus(
      base({ live: [{ slug: 'backend', healthUrl: 'https://api.example.com/health', probe: { status: 204, version: 'oldsha00' }, expectedSha: 'abc123def456' }] }),
    )
...
describe('stack lock', () => {
  it('a stale lock warns and points at unlock', () => {
    const report = evaluateStatus(
      base({ lock: { held: true, owner: 'ci:run-5', operation: 'deploy', expiresAt: '2020-01-01T00:00:00Z', stale: true } }),
    )
  ...
  it('a live lock warns without an action (a run may be in progress)', () => {
    const report = evaluateStatus(
      base({ lock: { held: true, owner: 'ci:run-5', operation: 'deploy', expiresAt: '2999-01-01T00:00:00Z', stale: false } }),
    )
    const check = find(report.checks, 'state.lock')
    expect(check?.status).toBe('warn')
