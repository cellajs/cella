...
import { resolveProjectId } from '../scaleway/bootstrap-scw-env';
...
export async function loadStackContext(
  ...
  loadModeEnvFile(environment, log);
  ...
  return { environment, stackPath, stackYaml, state, appConfig, projectId: resolveProjectId() ?? '' };
}
