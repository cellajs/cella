...
export type RepairResult = { action: 'absent' } | { action: 'healthy' } | { action: 'repaired'; state: ControlState };
...
export async function repairControlStore(argv = process.argv.slice(2)): Promise<void> {
  ...
  for (const [slug, entry] of Object.entries(state.rollout)) {
    console.info(
      `  ${slug}: seq=${entry.seq}${entry.pendingSha ? ` pendingSha=${entry.pendingSha}` : ''}${entry.active ? ` active=${entry.active.id}` : ' (no active)'}`,
    );
  }
