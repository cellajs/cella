...
describe('sequenceCutover — start-first', () => {
  ...
  it('orders the steps health → expand → contract → drain', async () => {
    const order: string[] = [];
    const res = await sequenceCutover(
      lbPlan({
        healthGate: async () => {
    ...
    expect(res.ok).toBe(true);
    expect(order).toEqual(['health', 'expand', 'contract', 'drain']);
  });
    ...
    const getServers = createLbGetServers({
      ...
      fetchImpl: async (url) => {
        ...
        return {
          ok: true,
          status: 200,
          text: async () => JSON.stringify({ servers: [{ ip: '10.0.0.4' }, { server_ip: '10.0.0.9' }] }),
        };
      },
    });
  ...
  it('parses wrapped backend payloads from Scaleway', async () => {
    const getServers = createLbGetServers({
      ...
      fetchImpl: async () => ({
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ backend: { server_ip: ['10.0.0.4'] } }),
      }),
    });
  ...
  it('parses provider-style serverIps payloads', async () => {
    const getServers = createLbGetServers({
      ...
      fetchImpl: async () => ({
        ok: true,
        status: 200,
        text: async () => JSON.stringify({ backend: { serverIps: ['10.0.0.4'] } }),
      }),
    });
    await expect(getServers()).resolves.toEqual(['10.0.0.4']);
  });
