...
describe('assertVmGrants', () => {
  it('passes when the union of policy rules covers all required permission sets', async () => {
    const fetchImpl = makeFetch([
      NO_GROUPS,
      {
        ...
        body: { policies: [{ id: 'pol-1', name: 'vm-reader-policy', application_id: 'vm-app' }] },
      },
  ...
  it('tolerates an EXTRA read-only set as benign drift (warns, does not fail)', async () => {
    ...
    const fetchImpl = makeFetch([
      ...
      {
        match: '/iam/v1alpha1/rules?policy_id=pol-1',
        body: { rules: [{ permission_set_names: [...REQUIRED, 'ObjectStorageReadOnly'] }] },
      },
    ]);
  ...
  it('excludes policies bound to other principals (shared-organization leakage)', async () => {
    const fetchImpl = makeFetch([
      ...
      {
        match: '/iam/v1alpha1/rules?policy_id=pol-admin',
        body: { rules: [{ permission_set_names: ['OrganizationManager'] }] },
      },
      {
        match: '/iam/v1alpha1/rules?policy_id=pol-raak',
        body: { rules: [{ permission_set_names: ['SecretManagerFullAccess'] }] },
      },
    ]);
  ...
  it('includes policies bound to a group the application belongs to', async () => {
    const fetchImpl = makeFetch([
      ...
      {
        match: '/iam/v1alpha1/policies?',
        body: { policies: [{ id: 'pol-g', name: 'group-policy', group_id: 'grp-1' }] },
      },
  ...
  it('aggregates permission sets across multiple policies and rules', async () => {
    const fetchImpl = makeFetch([
      NO_GROUPS,
      {
        ...
        body: {
          policies: [
            { id: 'pol-1', name: 'a', application_id: 'vm-app' },
            { id: 'pol-2', name: 'b', application_id: 'vm-app' },
          ],
        },
      },
      {
