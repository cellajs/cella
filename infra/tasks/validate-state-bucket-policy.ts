...
export async function validateStateBucketPolicy(opts: ValidateOptions): Promise<PolicyCheck[]> {
  ...
  checks.push(await expectAllowed('operator: GetObject', () => operatorS3.send(new GetObjectCommand({ Bucket: bucket, Key: probeKey }))))
  checks.push(
    await expectAllowed('operator: PutObject', () => operatorS3.send(new PutObjectCommand({ Bucket: bucket, Key: probeKey, Body: 'probe-op-write' }))),
  )
  ...
  checks.push(
    await expectAllowed('ci: PutObject (state write)', () => ciS3.send(new PutObjectCommand({ Bucket: bucket, Key: probeKey, Body: 'probe-ci-write' }))),
  )
  // Plain DeleteObject on a versioned bucket is a recoverable delete marker; CI is allowed this.
  checks.push(await expectAllowed('ci: DeleteObject (recoverable marker)', () => ciS3.send(new DeleteObjectCommand({ Bucket: bucket, Key: probeKey }))))
  ...
  try {
    ...
    for (const v of [...(listed.Versions ?? []), ...(listed.DeleteMarkers ?? [])]) {
      if (v.Key && v.VersionId) await operatorS3.send(new DeleteObjectCommand({ Bucket: bucket, Key: v.Key, VersionId: v.VersionId }))
    }
    await operatorS3.send(new PutBucketVersioningCommand({ Bucket: bucket, VersioningConfiguration: { Status: 'Enabled' } }))
  } catch (err) {
...
export async function main(): Promise<void> {
  ...
  if (!opAccess || !opSecret) {
    throw new Error('SCW_OPERATOR_ACCESS_KEY and SCW_OPERATOR_SECRET_KEY must be set (an API key on the operator IAM application)')
  }
