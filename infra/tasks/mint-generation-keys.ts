...
async function resolveAppId(secretKey: string, organizationId: string, name: string): Promise<string> {
  ...
  if (!app) throw new Error(`mint-generation-keys: IAM application '${name}' not found — run the infra CLI "Migrate IAM model" / bootstrap first.`)
  return app.id
}

/** Mint a fresh key on the app, then prune all but the newest KEYS_TO_KEEP. */
async function mintAndPrune(secretKey: string, organizationId: string, projectId: string, appId: string, label: string, sha: string, log: (msg: string) => void): Promise<ScwApiKey> {
  const fresh = await scwFetch<ScwApiKey>({ secretKey }, 'POST', `${IAM_BASE}/api-keys`, {
    application_id: appId,
    description: `${label} gen ${sha.slice(0, 10)}`,
    default_project_id: projectId,
  })
  const { api_keys = [] } = await scwFetch<{ api_keys?: ScwApiKey[] }>({ secretKey },
    'GET',
...
export async function mintGenerationKeys(opts: MintGenerationKeysOptions): Promise<GenerationKeys> {
  ...
  const bootKey = await mintAndPrune(opts.callerSecretKey, opts.organizationId, opts.projectId, bootAppId, names.boot, opts.sha, log)
  ...
  for (const service of opts.services) {
    ...
    const serviceKey = await mintAndPrune(opts.callerSecretKey, opts.organizationId, opts.projectId, appId, appName, opts.sha, log)
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (!secretKey || !sha || !outFile || !projectId || !organizationId) {
    throw new Error('Required: SCW_SECRET_KEY, --sha, --out, --project-id (or SCW_DEFAULT_PROJECT_ID), --organization-id (or SCW_DEFAULT_ORGANIZATION_ID)')
  }
