...
export function renderInstructions({ mode }: Args): string {
  return [
    ...
    `#    "ci-${mode}-deploy". Attach only the permission sets that`,
    ...
    '',
    '# 4. If the CI application id is stored in stack config for this fork,',
    '#    update it before running Pulumi:',
    `#      pulumi config set --stack ${mode} infra:ciApplicationId <application_id>`,
    `#      pulumi up --stack ${mode}`,
    '',
    '# 5. Trigger a deploy and confirm the rollout verification succeeds with the new key.',
    '',
