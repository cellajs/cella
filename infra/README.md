...
3. **Least-privilege credentials, per mode.** All principals are per app×mode (`<slug>-<mode>-…`), collected in one IAM group. The load-bearing boundary: **the CI key can never write IAM policy**, because granting permissions self-escalates to full admin (OWASP CICD-SEC-6 / NIST AC-6 territory), so identity administration stays a transient human action. The tiers:
