...
**Releases can't break what is running.** Every release starts on fresh servers and traffic only moves once the new version provably serves. (The `singleVM` cost mode trades that overlap for a short serving gap — see [Core philosophy](#core-philosophy).)
...
1. **Create-then-replace.** A release never mutates a running server. Each deploy provisions a new immutable **generation** per service, moves load-balancer traffic once the new generation provably serves the expected version, then destroys the displaced one. Rollback is a revert commit through the same forward path.
   ...
   The one exception is **`singleVM`**, the cost mode that folds every enabled worker onto the backend's VM. A stop-first worker in that fold (cdc, which owns a single replication slot) makes the whole host stop-first: the provisioning update replaces the one VM in place, so the old generation is gone before the new one serves. The deploy still health-gates before declaring success, but there is a serving gap instead of an overlap. Split-VM deploys — the default, and what cella runs — keep the overlap.
...
3. **Least-privilege credentials, per mode.** All principals are per app×mode (`<slug>-<mode>-…`), collected in one IAM group. Their canonical names ([lib/scaleway/principals.ts](lib/scaleway/principals.ts)) are the only identity contract: the Pulumi program, the CLI and `infra status` all resolve application ids from IAM by name, so no principal id is ever persisted to disk or carried in an environment variable. The load-bearing boundary: **the CI key can never write IAM policy**, because granting permissions self-escalates to full admin (OWASP CICD-SEC-6 / NIST AC-6 territory), so identity administration stays a transient human action. The tiers:
   ...
   - **CI deploy app**: project-scoped writes for routine deploys, plus one org-scoped `IAMApplicationManager` rule so it can rotate service keys every deploy. That rule is deliberately **unconditioned**: an api-key POST carries no `resource.id`, so a `resource.id in [service app ids]` condition 403s even for a listed app ([lib/scaleway/permissions.ts](lib/scaleway/permissions.ts)). The accepted widening is that CI can manage applications and keys org-wide; what still holds is the absent `IAMPolicyManager` — CI cannot grant permissions, so it cannot escalate.
...
The engine is plain Pulumi + the Scaleway SDK; there is no plugin framework to learn. App-owned registries under [config/](config/) declare services, sizing, routing, and secret wiring — [services.config.ts](config/services.config.ts) (services, replacement strategies, routes), [sizing.ts](config/sizing.ts), [stores.config.ts](config/stores.config.ts) (managed and external backing stores), [runtime-secrets.config.ts](config/runtime-secrets.config.ts) and [env-suppliers.config.ts](config/env-suppliers.config.ts) (secret wiring), [managed-keys.config.ts](config/managed-keys.config.ts), [health.config.ts](config/health.config.ts), [frontend-csp.config.ts](config/frontend-csp.config.ts), [telemetry.config.ts](config/telemetry.config.ts) and [general.config.ts](config/general.config.ts); resource modules under [resources/](resources/) are ordinary Pulumi programs you can read and change. The app description itself is injected through [config/engine-config.ts](config/engine-config.ts), which keeps the engine decoupled from any one workspace.
