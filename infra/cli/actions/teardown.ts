...
export async function runTeardown(context: InfraContext): Promise<void> {
  ...
  if (mode === 'production') {
    console.warn(`${warningMark} ${pc.bold('This is PRODUCTION.')} Protected resources (frontend/private buckets, DB) will refuse destruction unless 'protect' is lifted in code — that refusal is deliberate.`)
  }
  ...
  const accessKey = await envOr(['SCW_TEARDOWN_ACCESS_KEY', 'SCW_BOOTSTRAP_ACCESS_KEY'], () => promptRequiredInput('Scaleway teardown access key'))
  const secretKey = await envOr(['SCW_TEARDOWN_SECRET_KEY', 'SCW_BOOTSTRAP_SECRET_KEY'], () => maskedSecret({ message: 'Scaleway teardown secret key' }))
  ...
  try {
    ...
    console.warn(`${warningMark} Could not resolve organization id (${errorMessage(error)}); IAM cleanup will be skipped.`)
  }

  pulumiLoginAndSelect(infraDir, env, appConfig, targetStack)

  const stackLock = await acquireStackLockOrExit({ appConfig, accessKey, secretKey, stack: targetStack, operation: 'teardown' })
  ...
  const destroy = spawnSync('pulumi', ['destroy', '--refresh', '--yes', '--stack', targetStack], { cwd: infraDir, env, stdio: 'inherit' })
  ...
  try {
    ...
    console.warn(`${warningMark} IAM cleanup incomplete: ${errorMessage(error)}. The remainder can be deleted in the console (group ${confirmToken}).`)
  }
