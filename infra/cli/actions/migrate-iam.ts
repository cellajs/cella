...
export async function runMigrateIam(context: InfraContext): Promise<void> {
  ...
  const phase = await select<'migrate' | 'cleanup' | 'back'>({
    ...
    loop: false,
    choices: [
      { name: 'Migrate to v2', value: 'migrate', description: 'Create per-mode/per-service principals, rotate the CI key, flip the stack to iamModel=v2.' },
      { name: 'Clean up legacy principals', value: 'cleanup', description: 'AFTER a green v2 deploy: delete the legacy ci-deploy/vm-reader/operator/s3 apps.' },
  ...
  const secretKey = await envOr('SCW_BOOTSTRAP_SECRET_KEY', () => maskedSecret({ message: 'Scaleway bootstrap secret key' }))
  const organizationId = await resolveOrganizationId(secretKey, context.projectId)

  if (phase === 'cleanup') {
    const legacyNames = [names.legacy.ciDeploy, names.legacy.vmReader, names.legacy.operator, `${appConfig.slug}-s3`, `${appConfig.slug}-${mode}-s3`, names.vmReader]
    console.info(`\nWill delete (when present): ${legacyNames.join(', ')}`)
    if (!(await confirm({ message: 'Proceed? Only do this after a v2 deploy verified green (old generations lose their keys).', default: false }))) return
  ...
  const env = buildProviderEnv(infraDir, { accessKey, secretKey, projectId: context.projectId, passphrase, organizationId })
  ...
  const synced = await syncGithubEnvironment({
    ...
    environment: mode,
    ciKey: { accessKey: ciKey.accessKey, secretKey: ciKey.secretKey, projectId: context.projectId, organizationId: ciKey.organizationId },
    passphrase,
  })
  console.info(`  ${checkMark} minted ${ciKey.accessKey}${synced ? ' → GitHub Environment synced' : ` — ${pc.yellow('GitHub sync failed; update SCW_* secrets manually')}`}`)

  console.info(`\n→ Stack config infra:iamModel = v2`)
  pulumiLoginAndSelect(infraDir, env, appConfig, targetStack)
  const set = spawnSync('pulumi', ['config', 'set', 'infra:iamModel', 'v2', '--stack', targetStack], { cwd: infraDir, env, stdio: 'inherit' })
  if (set.status !== 0) {
    console.warn(`  ${warningMark} could not set stack config — set it manually: pulumi config set infra:iamModel v2 --stack ${targetStack}`)
  }
