  ...
  context.state === 'fresh'
    ...
    : await select<CliMode>({
        ...
        choices: [
          ...
          { name: 'Bake compute image', value: 'bake', description: 'Build the boot agent + bake the Docker/Node/agent VM image with Packer. Required once before the first compute deploy; re-run to ship agent/image changes.' },
