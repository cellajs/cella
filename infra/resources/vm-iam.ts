...
function resolvePrincipalId(preferred: string, legacy: string): pulumi.Output<string | undefined> {
  return pulumi.all([findApplicationId(preferred), findApplicationId(legacy)]).apply(([modern, legacyId]) => {
    ...
    if (legacyId) {
      pulumi.log.warn(
        `IAM application '${preferred}' not found; using legacy '${legacy}'. Run the infra CLI "Migrate IAM model" to adopt per-mode principals.`,
      );
      return legacyId;
    }
    return undefined;
...
export const ciDeployApplicationId = requirePrincipalId(
  resolvePrincipalId(names.ciDeploy, names.legacy.ciDeploy),
 ...
 */
export const adminApplicationId: pulumi.Output<string | undefined> = resolvePrincipalId(
  names.admin,
  names.legacy.operator,
).apply((id) => {
  const fromEnv =
    process.env.SCW_ADMIN_APPLICATION_ID?.trim() || process.env.SCW_OPERATOR_APPLICATION_ID?.trim() || undefined;
  if (id) return id;
 ...
 */
export const legacyS3ApplicationId: pulumi.Output<string | undefined> = resolvePrincipalId(
