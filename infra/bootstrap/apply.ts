...
import { scwConfigPathNone } from '../lib/bootstrap-scw-env.js'
...
export async function runApplyMode(context: BootstrapContext): Promise<void> {
  ...
  const targetStack = await input({ message: 'Pulumi stack name', default: `organization/infra/${context.stackShort}` })
  ...
  spawnSync('pulumi', ['login', loginUrl], { cwd: context.infraDir, env: applyEnv, stdio: 'inherit' })
  spawnSync('pulumi', ['stack', 'select', targetStack], { cwd: context.infraDir, env: applyEnv, stdio: 'ignore' })
  ...
  try {
    ...
    swapped = true
    while (true) {
      const code = await runPulumiUpWithHint(targetStack, context.infraDir, applyEnv)
      if (code === 0) break
