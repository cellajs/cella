    ...
    <div className="mt-16 max-w-screen-lg mx-auto">
      ...
      <ul className="mt-10">
        {entries.map(([id, module]) => {
          const authors = module.frontmatter.authors.map(
            (author) => AUTHORS[author],
          );
          console.log("module.frontmatter.image", module.frontmatter.image);
          return (
            <li className="text-lg mt-2 md:mt-1">
              <a href={`${id.replace(/\.mdx$/, "").replace(/\./g, "")}`}>
                <div className="flex justify-between items-center relative">
                  <p className="text-gray-400 text-md font-bold">
                    READING TIME・{module.frontmatter.readingTime}
                  </p>
