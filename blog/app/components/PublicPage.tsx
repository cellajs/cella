  ...
  },
  {
    ...
    links: [
      { title: "common:api_docs", href: `${config.backendUrl}/docs` },
      {
        ...
        href: "https://github.com/cellajs/cella/blob/main/info/ARCHITECTURE.md",
      },
      {
        title: "common:roadmap",
        href: "https://github.com/cellajs/cella/blob/main/info/ROADMAP.md",
      },
    ],
  },
  {
    ...
    <nav>
      <ul className="grid grid-cols-2 gap-8 sm:grid-cols-3">
        {footerSections.map((section) => (
          <li
            key={section.title}
            className={section.hideOnMobile ? "hidden sm:block" : ""}
          >
            <div className="font-display text-sm font-semibold tracking-wider text-white/40">
              {t(section.title)}
            </div>

            <ul className="mt-4 text-sm text-white/90">
              {section.links.map((link) => {
                const target = link.href.startsWith("http")
                  ? "_blank"
                  : "_self";
                return (
...
export const MainLayout: FC<Props> = ({ children }) => {
  return (
    <MarketingPage
      title="Blog"
      Link={<RenderNavItems />}
      AboutLink={<AbountFooter />}
      LegalLinks={<PublicFooter />}
