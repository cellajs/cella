...
An authorization request lands the browser on `/auth/consent?uid=…`, an app page under the sign-in framing. The page reads the interaction through the worker's JSON routes with the session cookie (a missing session goes through sign-in and back), shows who asks, the tenant (and on the MCP face the organization) the grant reaches, and the requested scopes as labels, and posts accept or refuse. The server names the tenant and organization, each only to a member of it. A registered app appears with the name and logo a system admin set; a client identified by its metadata document appears as the host serving its `client_id`, with no logo, since its author writes that document and would learn every viewer's address from a logo. Three refusals are decided server-side, by the grant policy (`grant-policy.ts`); the page shows the reason and disables Accept, and a posted accept is refused all the same: the person is not a member of the resource's tenant, the tenant does not allow consented clients (`restrictions.allowUnregisteredClients`), or the registered app is not installed there (`service_accounts.oauthClientId`).

The provider keeps a session of its own in the browser, and answers a client at once, without the consent page, only while that browser's app session belongs to the same person. Another person signed in there, or nobody, goes through the page, and consenting there never extends someone else's grant. Signing out of the app, ending one's other sessions or turning on MFA deletes the person's provider sessions; the grants and refresh tokens stay.
