...
export interface DataGridProps<R, SR = unknown, K extends Key = Key> extends SharedDivProps {
   ...
   * - 'multi': Multiple rows can be selected (default)
   * @default 'multi'
   */
...
export function DataGrid<R, SR = unknown, K extends Key = Key>(props: DataGridProps<R, SR, K>) {
  ...
  const selectionMode: SelectionMode = rawSelectionMode ?? 'multi';
  ...
  const headerSelectionValue = useMemo((): HeaderRowSelectionContextValue => {
    // In 'none' or 'single' mode, header selection is disabled
  ...
  function selectHeaderRow(args: SelectHeaderRowEvent) {
    ...
    // In 'none' or 'single' mode, header selection (select all) is disabled
  ...
  function selectRow(args: SelectRowEvent<R>) {
    if (!onSelectedRowsChange) return;
    // In 'none' mode, row selection is disabled
    ...
    // In 'single' mode, only allow one row to be selected
