...
OTel-based observability with [Maple.dev](https://maple.dev) as the telemetry backend. A shared `createOtelSDK()` factory configures traces, metrics, and logs identically for all Node.js services (backend, CDC, YJS). The frontend uses a browser `WebTracerProvider` for trace propagation via `traceparent` headers — no OTLP export from the browser.

Logging uses Pino with a `pino-opentelemetry-transport` bridge that forwards structured logs to the OTel pipeline in production. Health endpoints on backend (`/health`) and YJS (`/health`) expose service status, database connectivity, and CDC health. All services use a shared `setupGracefulShutdown()` for consistent lifecycle management with OTel flush on exit.

See [OTEL.md](./OTEL.md) for the full observability architecture.
