...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'cella_development_cdc_pub') THEN
    CREATE PUBLICATION cella_development_cdc_pub FOR TABLE users, organizations, attachments, pages, repositories, deployments, domains, requests, memberships;
...
ALTER TABLE repositories REPLICA IDENTITY FULL;
