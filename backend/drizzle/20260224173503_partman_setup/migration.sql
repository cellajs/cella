...
BEGIN
  ...
  -- ==========================================================================
  -- SEEN_COUNTS TABLE: Convert to partitioned (using LIKE clause)
  -- ==========================================================================
  -- Uses LIKE to clone existing table structure, making it robust to schema changes
  
  -- 1. Create partitioned table cloning structure from Drizzle-created table
  -- Note: We create the partitioned version first, then swap
  EXECUTE 'CREATE TABLE seen_counts_partitioned (LIKE seen_counts INCLUDING DEFAULTS INCLUDING CONSTRAINTS) PARTITION BY RANGE (entity_created_at)';
  
  -- 2. Clone indexes from original table
  FOR r IN 
    SELECT indexdef 
    FROM pg_indexes 
    WHERE tablename = 'seen_counts' 
    AND indexname NOT LIKE '%_pkey'
  LOOP
    -- Replace table name in index definition
    EXECUTE replace(r.indexdef, ' ON seen_counts ', ' ON seen_counts_partitioned ');
  END LOOP;
  
  -- 3. Setup pg_partman (1 week partitions, 4 weeks ahead)
  PERFORM partman.create_parent(
    p_parent_table => 'public.seen_counts_partitioned',
    p_control => 'entity_created_at',
    p_interval => '1 week'
  );
  ...
  UPDATE partman.part_config SET
    ...
    retention_keep_table = false,
    infinite_time_partitions = true
  WHERE parent_table = 'public.seen_counts_partitioned';
  
  -- 5. Migrate existing data
  INSERT INTO seen_counts_partitioned SELECT * FROM seen_counts;
  
  -- 6. Swap tables atomically
  ALTER TABLE seen_counts RENAME TO seen_counts_old;
  ALTER TABLE seen_counts_partitioned RENAME TO seen_counts;
  
  -- 7. Update partman config to use new table name
  UPDATE partman.part_config SET parent_table = 'public.seen_counts' WHERE parent_table = 'public.seen_counts_partitioned';
  
  -- 8. Drop old table
  DROP TABLE seen_counts_old;
  
  RAISE NOTICE 'seen_counts table converted to partitioned (via LIKE clause)';
