...
AS $$
BEGIN
  IF OLD.channel_type <> 'organization' OR OLD.role <> 'admin' THEN
    RETURN NULL;
  END IF;
  IF TG_OP = 'UPDATE' AND NEW.role = 'admin' AND NEW.channel_id = OLD.channel_id THEN
    RETURN NULL;
  END IF;
...
DROP TRIGGER IF EXISTS memberships_keep_org_admin ON memberships;
--> statement-breakpoint

CREATE CONSTRAINT TRIGGER memberships_keep_org_admin
  AFTER UPDATE OF role, channel_id OR DELETE ON memberships
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE FUNCTION memberships_keep_org_admin();
--> statement-breakpoint
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  BEGIN
    ...
    GRANT SELECT, INSERT, UPDATE, DELETE ON passkey_challenges TO runtime_role;
...
BEGIN
  ...
  -- Policy contract (3 tables x 4 policies)
  IF NOT EXISTS (
  ...
  IF NOT has_table_privilege('runtime_role', 'public.passkey_challenges', 'SELECT') THEN
    missing := array_append(missing, 'grant:passkey_challenges:SELECT'); END IF;
  IF NOT has_table_privilege('runtime_role', 'public.passkey_challenges', 'INSERT') THEN
    missing := array_append(missing, 'grant:passkey_challenges:INSERT'); END IF;
  IF NOT has_table_privilege('runtime_role', 'public.passkey_challenges', 'UPDATE') THEN
    missing := array_append(missing, 'grant:passkey_challenges:UPDATE'); END IF;
  IF NOT has_table_privilege('runtime_role', 'public.passkey_challenges', 'DELETE') THEN
    missing := array_append(missing, 'grant:passkey_challenges:DELETE'); END IF;
