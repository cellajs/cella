...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
    ...
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, context_id, context_type, organization_id, user_id', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  IF NEW.tenant_id IS DISTINCT FROM OLD.tenant_id
       ...
       OR NEW.context_type IS DISTINCT FROM OLD.context_type
       OR NEW.organization_id IS DISTINCT FROM OLD.organization_id THEN
    RAISE EXCEPTION 'Cannot modify immutable columns (%) on %', 'tenant_id, context_id, context_type, organization_id', TG_TABLE_NAME;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
