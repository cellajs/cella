...
export const SystemInviteEmail = ({ name, lng, senderName, inviteLink }: SystemInviteEmailProps) => {
  return (
    <EmailContainer previewText={i18n.t("backend:email.system_invite.preview", { appName, lng })}>
      ...
      <EmailHeader headerText={<div dangerouslySetInnerHTML={{ __html: i18n.t("backend:email.system_invite.title", { appName, lng }) }} />} />
      <EmailBody>
        <Text>
          <p style={{ marginBottom: "4px" }}>{name && i18n.t("backend:email.hi", { lng, name })}</p>
          <span dangerouslySetInnerHTML={{ __html: i18n.t("backend:email.system_invite.text", { lng, appName, senderName }) }} />
        </Text>

        <EmailButton
          ButtonText={i18n.t("common:join", { lng })}
          href={inviteLink}
        />
