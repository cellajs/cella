...
const organizationsRoutes = app
    ...
    .add(getOrganizationsRouteConfig, async (ctx) => {
        ...
        const counts = db
            .select({
                ...
                admins: count(
                    sql`CASE WHEN ${membershipsTable.role} = 'ADMIN' THEN 1 ELSE NULL END`
                ).as("admins"),
    ...
    .add(getUsersByOrganizationIdRouteConfig, async (ctx) => {
        ...
        if (role) {
            membersFilters.push(
                eq(membershipsTable.role, role.toUpperCase() as MembershipModel["role"])
            )
        }
