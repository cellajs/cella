...
import { and, count, eq, isNotNull, isNull, sql, type SelectedFields, type SQL, type SQLWrapper } from 'drizzle-orm';
...
export const getMemberCountsQuery = (entityType: ContextEntityType) => {
  ...
  const inviteCountSubquery = db
    ...
    .from(tokensTable)
    .where(and(eq(tokensTable.type, 'invitation'), isNotNull(tokensTable.entityType), isNull(tokensTable.consumedAt)))
