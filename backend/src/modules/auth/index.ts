...
const authRoutes = app
        ...
        const emailHtml = render(
            VerificationEmail({
                i18n: i18n.cloneInstance({
                    lng: i18n.languages.includes(emailLanguage)
                        ? emailLanguage
    ...
    .add(acceptInviteRouteConfig, async (ctx) => {
        ...
        if (token.userId) {
            ...
            if (password) {
                await Promise.all([
                    db.delete(tokensTable).where(and(eq(tokensTable.id, verificationToken))),
                    setSessionCookie(ctx, user.id, "password")
                ])
            }
        } else {
            return errorResponse(ctx, 400, "invalid_token", "warn", undefined, {
        ...
        if (oauth === "github") {
            const response = await fetch(
                `${config.backendUrl + githubSignInRouteConfig.route.path}${
                    organization ? `?redirect=${organization.slug}` : ""
                }`,
                {
