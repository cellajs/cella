...
app.openapi(authTotpsRoutes.generateTotpKey, async (ctx) => {
  ...
  const existingTotp = await findExistingTotp(baseDb, { userId: user.id });
...
app.openapi(authTotpsRoutes.createTotp, async (ctx) => {
  ...
  const existingTotp = await findExistingTotp(db, { userId: user.id });
  if (existingTotp) throw new AppError(409, 'resource_already_exists', 'warn');
  ...
  await insertTotp(db, { userId: user.id, secret: encodedSecret });
