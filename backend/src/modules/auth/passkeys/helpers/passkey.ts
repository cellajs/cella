...
/**
 * Verifies a passkey (WebAuthn) registration response: attestation, relying-party ID, origin, challenge, and user presence.
 * Returns the credential ID, public key, and signature counter to store (base64url, COSE key as base64url).
 */
...
/** Validates a passkey assertion for `userId`: consumes the challenge cookie, loads the stored credential, verifies the signature, and persists the new counter. */
