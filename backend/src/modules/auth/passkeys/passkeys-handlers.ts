...
app.openapi(authPasskeysRoutes.generatePasskeyChallenge, async (ctx) => {
  ...
  if (email && type === 'authentication') {
    ...
    user = await findUserByEmail(db, { email: normalizedEmail });
  }
  ...
  const credentials = await findCredentialIdsByUser(db, { userId: user.id });
...
app.openapi(authPasskeysRoutes.signInWithPasskey, async (ctx) => {
  ...
  if (!user) {
    const passkeyRecord = await findUserByCredentialId(db, { credentialId: passkeyData.credentialId });

    if (passkeyRecord) {
      user = await findAuthUserById(db, { userId: passkeyRecord.userId });
    }
  }
