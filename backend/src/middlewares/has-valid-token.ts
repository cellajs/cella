...
export const hasValidToken = (requiredType?: TokenModel['type']): MiddlewareHandler => {
  return async (ctx, next) => {
    ...
    if (isExpiredDate(tokenRecord.expiresAt)) return errorResponse(ctx, 401, 'expired_token', 'warn');

    // Check if token type matches the required type (if specified)
    if (requiredType && tokenRecord.type !== requiredType) return errorResponse(ctx, 401, 'invalid_token', 'warn');
