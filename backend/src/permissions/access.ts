...
export interface AccessContext {
  var: {
    actor?: Pick<ContextActor, 'id' | 'grants'>;
...
export const actorFrom = (ctx: AccessContext): Actor =>
  ctx.var.actor ? { userId: ctx.var.actor.id, isSystemAdmin: ctx.var.isSystemAdmin } : { anonymous: true };
...
export const accessFrom = (ctx: AccessContext): Access<MembershipBaseModel> =>
  ctx.var.actor
    ? { userId: ctx.var.actor.id, isSystemAdmin: ctx.var.isSystemAdmin === true, memberships: ctx.var.actor.grants }
    : { anonymous: true };
