...
const cdcMessageSchema = z.object({
  ...
  batchRows: z
    ...
    .optional(),
  // Cache token generated by CDC for single-entity realtime messages (null for batches)
...
class CdcWebSocketServer {
  ...
  private handleMessage(data: string): void {
    try {
      ...
      // Keep the single-entity detail cache fresh on writes.
      const entityType = message.activity.entityType;
      if (message.batchRows?.length && entityType) {
        // Batch messages carry no cache token; invalidate each affected entity's detail
        // entry from its row id so a later detail fetch re-enriches instead of serving stale.
      ...
      } else if (message.cacheToken && entityType && message.activity.subjectId) {
        // Single entity: reserve the token → entity key and mark the cached value stale.
        entityCache.reserve(message.cacheToken, entityType, message.activity.subjectId);
      }
