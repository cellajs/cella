...
import { membershipInvite, signIn } from 'sdk';
import { afterEach, beforeAll, describe, expect, it, vi } from 'vitest';
import { baseDb as db } from '#/db/db';
import { inactiveMembershipsTable } from '#/db/schema/inactive-memberships';
import { defaultHeaders } from '../fixtures';
import { createOrganizationAdminUser, createPasswordUser, createTestOrganization } from '../helpers';
...
describe('Membership Invitation', async () => {
  ...
  it('should invite existing users to organization', async () => {
    const { organization, sessionCookie } = await createOrgAndAdmin();
    const existingUser = await createPasswordUser('existing@example.com', 'password123!');
