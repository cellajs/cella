...
import { signIn, systemInvite } from 'sdk';
import { afterEach, beforeAll, describe, expect, it, vi } from 'vitest';
import { baseDb as db } from '#/db/db';
import { tokensTable } from '#/db/schema/tokens';
import { defaultHeaders } from '../fixtures';
import { createPasswordUser, createSystemAdminUser } from '../helpers';
...
describe('System Invitation', async () => {
  ...
  async function createAdminSession() {
    await createSystemAdminUser('admin@example.com', 'adminPassword123!');
  ...
  describe('Basic Functionality', () => {
    ...
    it('should filter out existing users', async () => {
      await createPasswordUser('existing@example.com', 'password123!');
      const sessionCookie = await createAdminSession();
      const { response: res, data } = await makeInviteRequest(
  ...
  describe('Input Validation', () => {
    it('should reject empty email list', async () => {
      const sessionCookie = await createAdminSession();
      const { response: res } = await makeInviteRequest([], sessionCookie);
      expect(res.status).toBe(403);
    });

    it('should reject invalid email formats', async () => {
      const sessionCookie = await createAdminSession();
      const { response: res } = await makeInviteRequest(['invalid-email', 'user@example.com'], sessionCookie);
      expect(res.status).toBe(403);
    });
  });
