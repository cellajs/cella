...
const TEST_ORG_C = 'rls_test_org_c_001'; // Second org in Tenant A (no membership for User A)
...
const TEST_PROJECT_A = 'rls_test_proj_a_001';
const TEST_TASK_A = 'rls_test_task_a_001';
const TEST_LABEL_A = 'rls_test_lbl_a_001';
const TEST_ATTACHMENT_A = 'rls_test_att_a_001';
const TEST_ATTACHMENT_C = 'rls_test_att_c_001'; // Attachment in Org C (User A has no membership)
...
async function setupTestData() {
  ...
  await adminDb.execute(sql`
    INSERT INTO pages (id, entity_type, tenant_id, name, stx, keywords, created_by, display_order, public_at)
    VALUES
      (${TEST_PAGE_A}, 'page', ${TEST_TENANT_A}, 'Private Page A', '{}', '', ${TEST_USER_A}, 1, null),
      (${TEST_PAGE_B}, 'page', ${TEST_TENANT_B}, 'Private Page B', '{}', '', ${TEST_USER_B}, 1, null),
      (${TEST_PAGE_PUBLIC}, 'page', ${TEST_TENANT_A}, 'Public Page A', '{}', '', ${TEST_USER_A}, 2, NOW())
    ON CONFLICT (id) DO NOTHING
  `);
...
async function cleanupTestData() {
  await adminDb.execute(sql`DELETE FROM yjs_documents WHERE entity_id LIKE 'rls_test_%'`);
  await adminDb.execute(sql`DELETE FROM tasks WHERE id LIKE 'rls_test_%'`);
  await adminDb.execute(sql`DELETE FROM labels WHERE id LIKE 'rls_test_%'`);
  await adminDb.execute(sql`DELETE FROM attachments WHERE id IN (${TEST_ATTACHMENT_A}, ${TEST_ATTACHMENT_C})`);
  await adminDb.execute(sql`DELETE FROM attachments WHERE id LIKE 'rls_wt_%'`);
...
describe('RLS Policy Verification', () => {
  ...
  describe('Cross-tenant write isolation', () => {
    ...
    it('should allow inserting page into any tenant (no RLS on pages)', async () => {
      const fakePageId = 'rls_fake_page_001';
      ...
      await queryAsRuntimeRole(TEST_TENANT_A, TEST_USER_A, async (tx) =>
        tx.execute(sql`
            ...
            VALUES (${fakePageId}, 'page', ${TEST_TENANT_B}, 'Fake Page', '{}', '', ${TEST_USER_A}, 99)
          `),
      );
      // Cleanup
    ...
    it('should allow inserting membership into any tenant (no RLS on memberships)', async () => {
      ...
      await queryAsRuntimeRole(TEST_TENANT_A, TEST_USER_A, async (tx) =>
        tx.execute(sql`
            INSERT INTO memberships (id, tenant_id, context_type, context_id, user_id, role, created_by, display_order, organization_id)
            VALUES ('rls_fake_mem', ${TEST_TENANT_B}, 'organization', ${TEST_ORG_B}, ${TEST_USER_A}, 'member', ${TEST_USER_A}, 99, ${TEST_ORG_B})
          `),
      );
      // Cleanup
      await adminDb.execute(sql`DELETE FROM memberships WHERE id = 'rls_fake_mem'`);
    });
    ...
    it('should allow deleting pages in any tenant (no RLS on pages)', async () => {
      // Create a temp page to delete (don't delete test data)
      const tempPageId = 'rls_temp_del_page';
      await adminDb.execute(sql`
        INSERT INTO pages (id, entity_type, tenant_id, name, stx, keywords, created_by, display_order)
        VALUES (${tempPageId}, 'page', ${TEST_TENANT_B}, 'Temp Delete Page', '{}', '', ${TEST_USER_B}, 99)
        ON CONFLICT (id) DO NOTHING
      `);
  ...
  describe('Unauthenticated write denial', () => {
    it('should allow page insert when unauthenticated (no RLS on pages)', async () => {
      const unauthPageId = 'rls_unauth_page_01';
      // No RLS on pages — insert succeeds (sysAdminGuard prevents this at API layer)
      await queryAsRuntimeRole(TEST_TENANT_A, '', async (tx) =>
        tx.execute(sql`
            ...
            VALUES (${unauthPageId}, 'page', ${TEST_TENANT_A}, 'Unauth Page', '{}', '', 99)
          `),
      );
      // Cleanup
    ...
    it('should allow membership insert without authentication (no RLS on memberships)', async () => {
      ...
      await queryAsRuntimeRole(TEST_TENANT_A, '', async (tx) =>
        tx.execute(sql`
            INSERT INTO memberships (id, tenant_id, context_type, context_id, user_id, role, created_by, display_order, organization_id)
            VALUES ('rls_unauth_mem', ${TEST_TENANT_A}, 'organization', ${TEST_ORG_A}, ${TEST_USER_B}, 'member', ${TEST_USER_B}, 99, ${TEST_ORG_A})
          `),
      );
      // Cleanup
      await adminDb.execute(sql`DELETE FROM memberships WHERE id = 'rls_unauth_mem'`);
    });
  });
  ...
  describe('Composite foreign key enforcement', () => {
    it('should reject inserting a task with mismatched tenant_id / organization_id', async () => {
      ...
      await expect(
        adminDb.execute(sql`
          INSERT INTO tasks (id, entity_type, tenant_id, name, stx, keywords, summary, variant, display_order, status, organization_id, project_id, created_by)
          VALUES ('rls_fk_task', 'task', ${TEST_TENANT_B}, 'FK Task', '{}', '', '', 1, 99, 5, ${TEST_ORG_A}, ${TEST_PROJECT_A}, ${TEST_USER_A})
        `),
    ...
    it('should reject inserting a label with mismatched tenant_id / organization_id', async () => {
      await expect(
        adminDb.execute(sql`
          INSERT INTO labels (id, entity_type, tenant_id, name, stx, keywords, organization_id, project_id, created_by)
          VALUES ('rls_fk_label', 'label', ${TEST_TENANT_B}, 'FK Label', '{}', '', ${TEST_ORG_A}, ${TEST_PROJECT_A}, ${TEST_USER_A})
        `),
      ).rejects.toThrow(/foreign key|violates/i);
    });
    ...
    it('should reject inserting an attachment with mismatched tenant_id / organization_id', async () => {
      await expect(
        adminDb.execute(sql`
          INSERT INTO attachments (id, entity_type, tenant_id, name, stx, keywords, created_by, organization_id, bucket_name, filename, content_type, size, original_key)
          VALUES ('rls_fk_att', 'attachment', ${TEST_TENANT_B}, 'FK File', '{}', '', ${TEST_USER_A}, ${TEST_ORG_A}, 'test-bucket', 'fk.txt', 'text/plain', '10', 'attachments/fk.txt')
        `),
      ).rejects.toThrow(/foreign key|violates/i);
    });

    it('should allow inserting with matching tenant_id / organization_id', async () => {
      const id = 'rls_fk_valid';
      await expect(
        adminDb.execute(sql`
          INSERT INTO tasks (id, entity_type, tenant_id, name, stx, keywords, summary, variant, display_order, status, organization_id, project_id, created_by)
  ...
  describe('Immutability triggers', () => {
    ...
    // Parentless product entities: base columns only
    ...
    it.each(allEntityCases)('should reject %s.%s mutation (%s)', async (tableName, column) => {
      ...
      await expect(
        adminDb.execute(sql.raw(`UPDATE ${tableName} SET ${column} = 'hacked_value' WHERE 1=1`)),
    ...
    it('should reject updates on append-only activities table', async () => {
      ...
      await expect(adminDb.execute(sql.raw("UPDATE activities SET id = 'hacked' WHERE 1=1"))).rejects.toThrow(
