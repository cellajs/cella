...
import { createTotp, generateTotpKey, signIn, signInWithTotp } from 'sdk';
import { appConfig } from 'shared';
import { afterEach, beforeAll, describe, expect, it, vi } from 'vitest';
import { baseDb as db } from '#/db/db';
import { totpsTable } from '#/db/schema/totps';
import { defaultHeaders, signUpUser } from '../fixtures';
import { createMfaToken, createPasswordUser, createTotpUser, enableMFAForUser, verifyUserEmail } from '../helpers';
...
describe('TOTP Authentication', async () => {
  ...
  describe('TOTP Sign-In Flow', () => {
    it('should sign in with valid TOTP code', async () => {
      const user = await createTotpUser(signUpUser.email, signUpUser.password);
