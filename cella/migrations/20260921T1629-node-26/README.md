...
3. App-owned Dockerfiles or version pins (`.nvmrc`, `volta` key, hosting settings): move `node:24-*` and `24` to `26`.
   Node 25+ no longer ships corepack, so a bare `RUN corepack enable` fails with exit code 127 on `node:26-*`:
   install it first, `RUN npm install -g corepack@latest && corepack enable` (the synced `Dockerfile` does this).
...
git grep -n -E "^RUN corepack enable" -- '*Dockerfile*'   # expect no output: corepack must be installed from npm first
pnpm check
pnpm test:core
```
