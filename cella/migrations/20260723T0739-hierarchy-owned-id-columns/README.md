...
The sync-breaking part: `appConfig.entityIdColumnKeys` is no longer hand-declared. The
template's `config.default.ts` now sets `entityIdColumnKeys: hierarchy.idColumnKeys`. An app
that keeps its own literal map still compiles, but drifts from the single source the moment
its hierarchy changes.

## Blast radius

Sync-breaking on config, not on call sites. No wire-shape change, no `clientCacheVersion`
bump, no database change (generated `path` column expressions are byte-identical). Apps that
...
below. Apps calling `pathColumnExpression(entityType, appendOwnId, h, idColumnKeys)` must
...
1. In your app's `config.default.ts`, replace the literal `entityIdColumnKeys: { ... }` map
...
2. If any app-specific code imports `pathColumnExpression` from
...
3. Optional cleanup: app-specific helpers that hand-write `` `${type}Id` `` or
