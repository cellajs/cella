...
organization** shared by all product entity types (reserved via `sequence` on the org's
`channel_counters` row; WAL order = sequence order). Every channel/product table gains a
...
2. **Product entity modules**: add `pathPrefix` to list ops (template pattern) and keep
   `seqCursor` semantics (values are now org-sequence; the endpoint shape is unchanged).
...
4. **Client**: nothing per-module — delta-fetch registrations keep working (they were
   ...
   the synced engine. `clientCacheVersion` was bumped upstream (`v2-sequence`); if your
...
recalculation (`pnpm seed counters` or your recalc runbook — it rebuilds `sequence`,
...
`tbl` discriminator so the partition spans all of them) — the sequence is shared. Then run
recalculation, which sets `sequence` to the max stamped value per org. Clients cache-bust
...
- De-hosted attachments stay de-hosted; `publicat_cascade` and partman retention are
  unaffected (the sequence is not the activity id).
