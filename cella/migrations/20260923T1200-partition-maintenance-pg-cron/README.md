...
image `backend/db/Dockerfile` ships pg_cron instead of pg_partman.
...
stack needs the new `admin-cron-privilege` on `rdb` applied before the next migrate. Apps that
only listed `appPartitionConfigs` need no code change.
...
3. Apps on Scaleway: CLI **Apply infra change** so the admin user gets `all` on the `rdb` database, before the next release migrates.
4. Any extra compose file that runs `backend/db` (devcontainers) adds `-c shared_preload_libraries=pg_cron -c cron.database_name=postgres`.
