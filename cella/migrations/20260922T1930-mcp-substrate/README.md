...
entity, execute }` on `createXRoute`, which registers it (`backend/src/core/mcp-tool-registry.ts`: `registerMcpTool`, `getMcpTools`, `McpTool` with `entity` + `action`). The input schema
