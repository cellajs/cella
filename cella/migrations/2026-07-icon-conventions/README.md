...
- `numericSizeProps`: `size={N}` on tags the codemod can't identify as icons —
  prop-aliased renders like `<Icon>` or `<item.icon>`. If the tag renders a
  lucide icon the prop is inert (it silently gets the 1rem default): move the
  size to a class. Non-icon components with their own `size` prop (e.g.
  `QRCodeSVG`) are false positives — leave those.
...
For icon-as-prop components, type the prop as `IconComponent` from
`~/modules/common/icons/types` instead of `LucideIcon` — it omits `size`, so
passing the inert prop becomes a type error instead of a silent no-op.
