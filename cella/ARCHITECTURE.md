...
Authentication supports magic links, passkeys, OAuth, and optional time-based one-time-password (TOTP). A magic link signs in directly only in the browser that asked for it; opened anywhere else it waits for a click on a confirmation page, so a link planted in someone's browser or fetched by an email scanner signs nobody in. A sign-up creates its account only once the inbox is proven: a sign-up link creates it when clicked, and an OAuth sign-up waits on its verification mail, completed by the same provider account in the browser that opened it. Every cookie is signed, bound to its name and expiry. Sessions are cookie-based and rate-limited: the cookie carries a random token and storage only its hash, so reading a sessions row yields no session. A system administrator's impersonation rides on a cookie of its own, on top of the admin's session, and holds only while that session and the admin's system role do. Account-security actions (factors, MFA, provider connect, OAuth consent, account deletion) need the session to step up first: a second factor the user holds, else a fresh sign-in or an emailed link opened in the same browser. A proof's row outlives the proof: a session or API key is revoked with a stamp (`revokedAt`, `revokedBy`, and for a session a `revocationReason`), a token is invoked, a consent code is consumed; reads filter on the stamp and the retention sweep removes the row later, so the sessions list can show what ended and why. Only a consent revoke deletes, because the authorization server owns those rows. Each sign-in enrolls the browser in the devices table under a per-user hash of a long-lived device cookie; a sign-in from a browser the account has not used before mails the owner, and rows unseen for 400 days are pruned. An account is identified by its sign-in credentials, not by an email address: an invitation sent to any address can be accepted by whoever opens its link, as the account they are signed in to. External identities (OAuth today, SSO later) live in the identities table keyed on kind, issuer slug and the issuer's subject, never on an address. The emails table lists the inboxes an account has proven, the sign-up address and any provider address whose verification mail was clicked; each is a magic-link sign-in identifier and an invitation target. Machines are actors too: a service account holds role bindings like a member and authenticates with an API key or an access token from the app's own authorization server, narrowed by access scopes ([Interoperability](./INTEROPERABILITY.md)). Cella has a layered approach to balance defense in depth, maintainability and performance.
