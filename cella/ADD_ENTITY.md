...
- **Scheduled jobs**: declare `jobs: [{ name, start }]` in `defineBackendModule` (`start` returns a stop handle). One API instance at a time runs them all: every instance with `RUN_JOBS` contends for a Postgres advisory lock (`backend/src/lib/job-ownership.ts`). Never edit `main.api.ts`.
