...
- **Scheduled jobs and queues**: declare `jobs: [{ name, cron, run }]` (pg-boss cron on a singleton queue, UTC) and `queues: [{ name, handler, ... }]` in `defineBackendModule`; the jobs service creates, schedules and works them ([Jobs service](../jobs/README.md)). Never edit `main.api.ts`.
