...
Here we give you a todo flow for adding a new entity model. First decide which kind you are adding (see [Architecture](./ARCHITECTURE.md#entity-hierarchy-model)):

- **Channel entity** (like `organization`): has memberships and roles. Access via app-layer guards; no RLS, no `seq`/`stx` sync.
- **Product entity** (like `attachment`): user-generated content, no memberships. RLS-protected tenant reads, full sync-engine participation.

The working principle: declare the entity in config and register it in a few registries. RLS grants, CDC publication, SSE fan-out, `seq` stamping, SDK types, query-key scopes and offline persistence are all derived from those registrations; there is no per-entity plumbing to write for them. Copy from `attachment` (product) or `organization` (channel) throughout.
