...
- **Frontend modules & placements**: every `frontend/src/modules/<name>/` folder registers itself in `<name>-module.ts` (`.tsx` when placements render JSX) via `defineFrontendModule` (`~/lib/module`); the composition root `frontend/src/modules.ts` glob-imports these files before first render, so app-owned modules register with no import list to maintain. A module declares UI contributions under `placements`; `~/lib/placements` indexes them per slot and hosting pages read them with `getPlacements(slot)`. Slot families: `` `${channelType}.settings.aside` `` and `account.settings.aside` (a titled card plus aside tab on that settings page, ordered with the built-ins via `order`, gated via `requires`). Page tabs are placements declared on the route itself via `staticData.navTab` (typed `PlacementTab`): adding a tab, including to the system panel, means adding a child route file, and `PageTabNav` discovers it from the route tree. Apps hide, reorder, or re-gate template defaults per host in `frontend/src/placement-config.ts` (pinned); hosts resolve final lists through `resolvePlacementList`, and a layout's default tab derives from the first visible tab (`defaultNavTabPath`). Keep module files light; lazy-load heavy UI inside placement renderers.
