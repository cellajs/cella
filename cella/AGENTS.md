...
- **Realtime frontend** in `frontend/src/query/realtime/`: Two streams, `AppStream` (authenticated, leader-tab via Web Locks + BroadcastChannel, echo prevention via `stx.sourceId`, catchup via declared views over the org sequence) and `PublicStream` (unauthenticated, per-tab connection, catches up deletes on connect then live-only).
