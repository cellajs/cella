...
- **Frontend modules & placements**: every `frontend/src/modules/<name>/` folder registers itself in `<name>-module.ts` via `defineFrontendModule` (`~/lib/module`); the composition root `frontend/src/modules.ts` glob-imports these files before first render, so app-owned modules register with no import list to maintain. A module declares UI contributions under `placements`; `~/lib/placements` indexes them per slot and hosting pages read them with `getPlacements(slot)`. Current slot family: `` `${channelType}.settings.aside` `` (a titled card plus aside tab on that entity's settings page, ordered with the built-ins via `order`, gated via `requires`). Page tabs are placements declared on the route itself via `staticData.navTab` (typed `PlacementTab`): adding a tab means adding a child route file, and `PageTabNav` discovers it from the route tree. Keep `*-module.ts` files light; lazy-load heavy UI inside placement renderers.
