...
- **Frontend modules & placements**: every `frontend/src/modules/<name>/` folder registers itself in `<name>-module.ts` (`.tsx` when tools render JSX) via `defineFrontendModule` (`~/lib/module`); the composition root `frontend/src/modules.ts` glob-imports these files before first render, so app-owned modules register with no import list to maintain. Vocabulary: a **tool** is a component placed into a **slot**; the **consumer** is the page hosting the slot and owning its contract. Modules declare tools under `tools`; consumers read them via typed getters (`getSettingsAsideTools(channelType)`, `getAccountSettingsTools()`) and resolve the final list with `resolvePlacementList`. Slot families: `` `${channelType}.settings.aside` `` and `account.settings.aside`; a tool's `render` returns the full card (lazy-load heavy UI) and its per-channel-type entity context comes from the `SettingsAsideEntityByType` interface, which apps widen via module augmentation. Gating: `requires` names a grant (capability condition), `visibleTo` lists context-role pairs like `'organization.admin'` (identity condition, matched over the entity's ancestor chain via `heldContextRoles`; a UI boundary only, never data authorization). Arrangement layers, applied in order: manifest defaults in code, app overrides in `frontend/src/placement-config.ts` (pinned), then the channel row's `toolsConfig` jsonb (per-slot `order`/`hidden`/`settings`, admin-edited, reconciled fail-closed: unknown ids drop, new tools append at their default order; `locked` tools ignore channel hiding). Page tabs are placements declared on the route via `staticData.navTab` (typed `PlacementTab`): adding a tab, including to the system panel, means adding a child route file; a layout's default tab derives from the first visible tab (`defaultNavTabPath`). A channel type's settings slot costs its forms plus one `channelSettingsTools(...)` call in its module and a `<ChannelSettingsPage entity={...} />` route component; shared shells are `SettingsToolCard`, `DangerZoneCard`, and `ToolsArrangementCard` (all in `modules/common` / `modules/entities`).
