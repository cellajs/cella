...
- **Sync signals** in `frontend/src/query/realtime/sync-signals.ts`: the supported extension point for per-user state derived from sync. `onChangeEvent` fires from the stream handler for every readable notification before any tier decision, so it also covers muted and archived channels; it carries ids only. `onSyncedRows` fires from the fetch prioritizer once a range has settled and carries the fetched rows, or an empty `degraded` batch when the range could not be delivered and the subscriber must invalidate instead of derive. The seen module subscribes to `onSyncedRows` for unseen tracking; app modules register their own subscribers the same way instead of importing their logic into the prioritizer.
...
- **Skills**: `cella/skills/` is the single home for agent skills (synced to apps). Claude Code only discovers skills under `.claude/skills`, which is gitignored — make them invocable with a local symlink: `ln -s ../cella/skills .claude/skills`.
...
- JSDoc on exports that need it. Backend: full JSDoc with params/response. Frontend: one line, and none at all when the identifier and its types already carry the meaning. `useAttachmentCreateMutation`, `attachmentQueryKeys`, and `OrganizationsGrid` earn no doc; `useAttachmentDeleteMutation` earns one, because it also cancels paused offline creates and drops those attachments locally without ever reaching the server. No standalone file-level comments above imports. A comment longer than three prose lines must directly document a declaration or local executable block. Move cross-file architecture, workflows, and failure-mode narratives to the nearest canonical README.
...
- **Comment budget.** Comments are the least reviewed and most copied lines in the codebase, so each one has to pay for itself:
  - **Members**: document an interface or object member when its name and type underdetermine the contract, and then in one line. Keep the doc when it states a default, a constraint, a unit or encoding, the condition under which the value is null or empty, or where the value is populated from; keep it whenever the type is `unknown`, `any`, or a bare `string`/`number`/`boolean`, which pin down nothing on their own. Drop it when a named type already carries the meaning (`items: FloatingNavItem[]`), or when the default and the behavior are both visible in the same file (`bodyClass?: string` destructured as `bodyClass = 'floating-nav'`). This is the opposite of the rule for functions: a function's signature and body reveal its behavior, so a doc repeating them is noise, while a `string` prop reveals nothing and its doc is often the only specification that exists.
  - **Locals**: one line for a local's rationale. A note that needs two lines is a request for a better name or an extracted function.
  - **JSX**: keep the constraint, drop the reasoning. `{/* min-h-14 matches the bar row so the grid holds position */}` is the whole comment; the measurement behind the number and the case that motivated it belong in the commit.
  - **No repeats**: the same comment text must not appear in two files. A module cloned from a sibling copies that sibling's comments with it. Put the note once at the shared abstraction, or delete it from every copy.
...
- Keep the content of existing code comments intact unless cleanup is explicitly requested. Trimming to the comment budget above is always in scope: a comment that breaks those limits is a defect in the file being touched, not a line to preserve.
