...
CI guard for the schema-evolution lens system (`shared/src/version-changes/`). Run via
...
4. **Entity-wire completeness**: every configured product/context entity type must register
   through the entity-wire factory in `backend/src/modules`, so an entity can never silently
   miss the lens seams (widening + normalize).
