...
non-null ancestor id. With nullable ancestors, rows may attach above their declared parent; the
deepest non-null ancestor is then the row's effective home. Without nullable ancestors this is just
the declared parent. The rule is exposed as `EntityHierarchy` instance methods
