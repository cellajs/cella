...
## More info
- Cella uses [Biome](https://biomejs.dev/). Please [install it](https://marketplace.visualstudio.com/items?itemName=biomejs.biome) for a correct code style. For manual lint fixing do `pnpm run check:fix` and to check the entire project for type errors: `pnpm run check:types`.
