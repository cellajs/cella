 ...
 * this to delete a document only after `ok` or `empty`. Unparseable state, or a log crediting no
 * editor, is never posted and counts as `permanent`.
 */
export async function compactDocument(scope: DocScope): Promise<CompactionResult> {
  ...
  const [editedBy] = editorsNewestFirst(rows);
  if (!editedBy) {
    log.error(`Compaction: no editor in the log of ${scope.entityType}:${scope.entityId}, keeping the log`);
    return 'permanent';
  }
  const result = await postMaterialize(scope, editedBy, json);
