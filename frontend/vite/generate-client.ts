...
const run = async () => {
  ...
  // Preserve the docs folder (generated by openapi-parser plugin separately)
  const docsFolder = resolve(finalOutputPath, 'docs');
  const tempDocsFolder = resolve(tempOutputPath, 'docs-backup');

  if (existsSync(docsFolder)) {
    cpSync(docsFolder, tempDocsFolder, { recursive: true });
  }
  ...
  // Restore docs folder
  if (existsSync(tempDocsFolder)) {
    cpSync(tempDocsFolder, docsFolder, { recursive: true });
  }
