...
export const schemaTagNames = [${schemaTags.map((t) => `'${t.name}'`).join(', ')}] as const;

export const schemaTags: { name: SchemaTag; description: string; count: number }[] = ${JSON.stringify(schemaTags, null, 2)};

export const schemas: GenComponentSchema[] = ${JSON.stringify(schemas, null, 2)};
`;
}
...
const handler: OpenApiParserPlugin['Handler'] = ({ plugin }) => {
  ...
  const getSchemaTag = (schemaName: string): 'base' | 'data' | 'errors' => {
...
export type { GenComponentSchema, GenInfoSummary, GenOperationDetail, GenOperationSummary, GenResponseSummary, GenTagSummary, SchemaTag } from '~/modules/docs/types';
