...
const DeviceMockup = ({ lightSlides, darkSlides, type, className }: DeviceMockupProps) => {
  ...
  const renderCarousel = ({ autoPlay = false, slide = 0 }) => {
    return (
      <Carousel
        opts={{ duration: 20, loop: true, startIndex: slide }}
        plugins={autoPlay ? [Autoplay({ delay: 4000, stopOnInteraction: true, stopOnMouseEnter: true })] : []}
          ...
          {slides?.map((slide, idx) => (
            <CarouselItem key={slide.src} onClick={() => onOpenChange(true, idx)}>
              <div className="h-full">
                <img src={slide.src} alt={`Slide ${idx}`} className="object-contain w-full h-full" />
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
        ...
        return (
          <div className="relative mx-auto border-gray-800 bg-gray-800 border-[14px] rounded-[2.5rem] aspect-[3/4]">
            ...
            <div className="rounded-[2rem] bg-white dark:bg-gray-800 h-full w-full cursor-pointer">
              {inView && renderCarousel({ autoPlay: true })}
            </div>
          </div>
        );
      case 'pc':
        return (
          <div className="w-full">
            <div className="relative mx-auto border-gray-400/75 mb-[1px] dark:border-gray-600 border-[8px] rounded-t-xl max-w-[85%] aspect-video">
              <div className="rounded-lg h-full w-full bg-background cursor-pointer">{inView && renderCarousel({ autoPlay: true })}</div>
            </div>
  ...
  return (
    <div className={cn(mockupClass, className)} ref={ref}>
      ...
      <Dialog open={isOpen} onOpenChange={onOpenChange}>
        <DialogContent className="min-w-full h-svh border-0 p-4 rounded-none">
          <DialogHeader>
            <DialogTitle className="text-center font-semibold text-lg">View screenshot</DialogTitle>
          </DialogHeader>
          <div className="flex flex-wrap justify-center">{renderCarousel({ autoPlay: false, slide: carouselSlide })}</div>
        </DialogContent>
      </Dialog>
    </div>
  );
