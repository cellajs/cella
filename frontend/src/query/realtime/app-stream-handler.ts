      ...
      cacheOps
        ...
        .then((success) => {
          if (success && notification.batchUntilSeq) {
            ...
            const seqKey = notification.contextId
              ? `${notification.contextId}:s:${entityType}`
              : `${organizationId}:s:${entityType}`;
            useSyncStore.getState().setSeq(seqKey, notification.batchUntilSeq);
          }
...
function handleEntityNotification(
  ...
  if (seq !== null) {
    const seqKey = contextId ? `${contextId}:s:${entityType}` : `${organizationId}:s:${entityType}`;
