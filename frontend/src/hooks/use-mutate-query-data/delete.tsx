...
export const useMutateDeleteData = (passedQueryKey: QueryKey, invalidateKeyGetter?: (item: ItemData) => QueryKey) => {
  return (items: ItemData[] | EntityData[], entity?: Entity) => {
    ...
    for (const [queryKey, queryData] of queriesToWorkOn) {
      ...
      if (isQueryData(queryData)) changeQueryData(queryKey, items, 'delete');

      if (isInfiniteQueryData(queryData)) changeInfiniteQueryData(queryKey, items, 'delete');

      if (entity && isArbitraryQueryData(queryData)) changeArbitraryQueryData(queryKey, items as EntityData[], 'delete', entity);
    }
