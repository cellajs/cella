 ...
 * 🛡️ Requires authentication
 *
 * **GET /entities/{idOrSlug}** ·· [getEntity](http://localhost:4000/docs#tag/entities/get/entities/{idOrSlug}) ·· _entities_
 *
 * @param {getEntityData} options
 * @param {string | string} options.path.idorslug - `string | string`
 * @param {enum} options.query.type - `enum`
 * @returns Possible status codes: 200, 400, 401, 403, 404, 429
 */
export const getEntity = <ThrowOnError extends boolean = true>(options: Options<GetEntityData, ThrowOnError>) => {
  return (options.client ?? _heyApiClient).get<GetEntityResponses, GetEntityErrors, ThrowOnError, 'data'>({
    responseStyle: 'data',
    security: [
      {
        in: 'cookie',
