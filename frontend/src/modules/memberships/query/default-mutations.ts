...
import type { InfiniteMemberQueryData, MemberContextProp, MemberQueryData } from './types';

const limit = config.requestLimits.members;

const errorHandler = (_: Error, __: UpdateMembershipProp | RemoveMembersProps, context?: MemberContextProp[]) => (queryClient: QueryClient) => {
  if (context?.length) {
    for (const [queryKey, previousData] of context) queryClient.setQueryData(queryKey, previousData);
  }
};

export const getDefaultMembershipUpdateMutation = (
  queryClient: QueryClient,
): DefaultMutationOptions<Membership, Error, UpdateMembershipProp, MemberContextProp[]> => {
  return {
    ...
    onError: (_, __, context) => errorHandler(_, __, context)(queryClient),
  };
};

export const getDefaultMembershipDeleteMutation = (
  queryClient: QueryClient,
): DefaultMutationOptions<void, Error, RemoveMembersProps, MemberContextProp[]> => {
  return {
