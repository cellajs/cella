...
import { credentialsQueryOptions, useRevokeCredentialMutation } from '~/modules/service-accounts/query';
...
export function ServiceAccountTile({ account, path }: ServiceAccountTileProps) {
  ...
  const live = (data?.items ?? []).filter((credential) => !credential.revokedAt);
  ...
  const handleRevoke = (credential: Credential) => {
    ...
    revoke({ path: { ...path, id: account.id, credentialId: credential.id } });
  };
  ...
  return (
    <Card className="w-full py-0 sm:py-0">
      <CardContent className="flex flex-col gap-2 p-3!">
        ...
        {live.map((credential) => (
          <div key={credential.id} className="flex items-center gap-3 text-sm">
