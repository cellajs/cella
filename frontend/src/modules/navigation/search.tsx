...
export const AppSearch = () => {
	...
	const updateRecentSearches = (value: string) => {
		...
		const hasSubstringMatch = recentSearches.some((element) =>
	...
	return (
		<Command className="rounded-lg shadow-2xl" shouldFilter={false}>
			...
			<ScrollArea
				id={"item-search"}
				ref={scrollAreaRef}
				className="sm:h-[40vh] overflow-y-auto"
			>
				{
										...
										{recentSearches.map((search, index) => (
											...
											>
												<div className="flex gap-2 items-center outline-0 ring-0 group">
													...
													<span className="underline-offset-4 truncate font-medium">
														{search}
													</span>
												</div>
												<div className="flex items-center">
													<span className="max-sm:hidden text-xs opacity-50 mx-3">
														{index}
													</span>
													<Button
														variant="ghost"
										...
										<CommandGroup className="">
											...
											{filteredItems.map((item: EntityListItemType) => {
												return (
													<CommandItem
														data-already-member={
															entityType !== "user" && item.membership !== null
														}
														key={item.id}
														disabled={
															entityType !== "user" && item.membership === null
													...
													>
														...
														<div className="flex items-center">
															...
															>
																<User size={14} />
																<span className="max-sm:hidden font-light">
																	{t("common:member")}
																</span>
															</Badge>
														</div>
