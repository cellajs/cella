...
import { type FilePanelProps, useBlockNoteEditor, useEditorState, useExtensionState } from '@blocknote/react';
...
const UppyFilePanel = ({ onComplete, onError, organizationId, isPublic = false }: BaseUppyFilePanelProps & FilePanelProps) => {
  ...
  const [open, setOpen] = useState(editor.getExtension(FilePanelExtension)?.store.state);
