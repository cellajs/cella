...
export const SheetMenuItem = ({ item, type, className, submenu, searchResults }: SheetMenuItemProps) => {
  ...
  return (
    ...
    >
      ...
      <AvatarWrap className={`${submenu ? 'my-2 mx-3 h-8 w-8 text-xs' : 'm-2'} z-[1] items-center`} type={type} id={item.id} name={item.name} url={item.thumbnailUrl} />
      <div className='truncate py-2 flex flex-col justify-center text-left'>
        <div className={`truncate ${submenu ? 'max-sm:pt-[6px] text-sm' : 'max-sm:pt-[10px] text-base'} leading-5`}>{item.name}</div>
