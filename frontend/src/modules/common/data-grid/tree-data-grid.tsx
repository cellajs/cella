...
export function TreeDataGrid<R, SR = unknown, K extends Key = Key>({
  ...
  function handleCellCopy(
    { row, column, rowIdx, value }: CellCopyArgs<NoInfer<R>, NoInfer<SR>>,
    event: CellClipboardEvent,
  ) {
    if (!isGroupRow(row)) {
      rawOnCellCopy?.({ row, column, rowIdx, value }, event);
    }
  }

  function handleCellPaste(
    { row, column, rowIdx, pastedValue }: CellPasteArgs<NoInfer<R>, NoInfer<SR>>,
    event: CellClipboardEvent,
  ) {
    return isGroupRow(row) ? row : rawOnCellPaste!({ row, column, rowIdx, pastedValue }, event);
  }
  ...
  return (
    <DataGrid<R, SR>
      ...
      columns={columns}
      rows={rows as R[]} // TODO-005: check types
      rowHeight={rowHeight}
