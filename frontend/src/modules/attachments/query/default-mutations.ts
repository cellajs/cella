...
import type { DefaultMutationOptions } from '~/query/types';
import { nanoid } from '~/utils/nanoid';

const limit = config.requestLimits.attachments;

const handleError = (action: 'create' | 'update' | 'delete' | 'deleteMany', queryClient: QueryClient, context?: AttachmentContextProp[]) => {
  if (context?.length) {
...
export const getDefaultAttachmentCreateMutation = (
  queryClient: QueryClient,
): DefaultMutationOptions<Attachment[], Error, CreateAttachmentParams, AttachmentContextProp[]> => {
  return {
    ...
    onError: (_, __, context) => handleError('create', queryClient, context),
  };
};

export const getDefaultAttachmentUpdateMutation = (
  queryClient: QueryClient,
): DefaultMutationOptions<Attachment, Error, UpdateAttachmentParams, AttachmentContextProp[]> => {
  return {
    ...
    onError: (_, __, context) => handleError('update', queryClient, context),
  };
};

export const getDefaultAttachmentDeleteMutation = (
  queryClient: QueryClient,
): DefaultMutationOptions<boolean, Error, DeleteAttachmentsParams, AttachmentContextProp[]> => {
  return {
    ...
    onError: (_, { ids }, context) => handleError(ids.length > 1 ? 'deleteMany' : 'delete', queryClient, context),
  };
};
