...
| URL resolution | `helpers/resolve-url.ts` | `resolveAttachmentUrl`: local blob first, cloud fallback, enqueues a background download. `resolveBlockNoteFileRef`: the same for editor blocks, after the media grammar (`shared/src/utils/media-ref.ts`) accepts the ref as an attachment id, a key under the document's organization or an asset URL; any other ref renders nothing. |
...
2. **BlockNote file panel** (`modules/common/blocknote/custom-file-panel/uppy-upload-panel.tsx`) for image/video/audio/file blocks: one file per block. The block stores the attachment id as its URL, or the cloud key when a public media mode's upload landed in the public bucket (the upload template decides, and the `attachment` upload template stores privately); the host form persists the parsed attachments under those ids (`helpers/persist-attachments.ts`) so block references stay valid. `public-no-attachment` persists no row: it uploads through its own public template (the newsletter's `newsletter` template, system admins only, stored under the system prefix) and the block keeps the stored key. Refs resolve through `blocknote/helpers/resolve-file-url.ts` (wraps `resolveBlockNoteFileRef`); the carousel opens imperatively (no URL binding).
...
Backend enforces everything: org admins full CRUD, members create/read (`shared/config/permissions-config.ts`); presigned-URL signing is permission-checked and fails closed; create and signing both refuse a key outside the organization's upload prefix (`<organizationId>/…`, which the upload token issues only to members) or a bucket other than the app's own; tenant quota (default 100 attachments) returns 429 on create. The frontend gates only inline rename, on `can.attachment.update`. Upload restrictions: `appConfig.uppy.defaultRestrictions`; local-cache behavior: `appConfig.localBlobStorage` (enabled flag, per-file/total size caps, content-type filters, concurrency, retry delays).
