...
| Table | `table/` | Org grid (route `/$tenantId/$orgSlug/organization/attachments`): thumbnail with upload-status badge, inline rename, description cell opening the editor sheet, per-row cloud download, bulk delete with confirmation, seen-marking via row visibility. |
