...
/** One account settings section: aside tab descriptor plus the rendered card, sorted on `order`. */
type AccountSection = PlacementTab & { order: number; node: ReactNode };
...
function UserAccountPage() {
  ...
  const contributionSections: AccountSection[] = getPlacements('account.settings.aside').map((placement) => ({
    ...placement,
    order: placement.order ?? 50,
    node: (
      <Card className="mx-auto sm:w-full" id={placement.id}>
        <CardHeader>
          <CardTitle>{t(placement.label, { resource: t(placement.resource || '').toLowerCase() })}</CardTitle>
        </CardHeader>
        <CardContent>{placement.render(user)}</CardContent>
      </Card>
    ),
  }));

  // Overrides in ~/placement-config can hide or reorder any entry; this page passes no grants
  const sections: AccountSection[] = resolvePlacementList('account.settings.aside', [
    {
      id: 'general',
      label: 'c:general',
      order: 10,
    ...
    // Authentication only renders when at least one strategy is enabled
  ...
  return (
    <div className="container my-4 gap-4 md:mt-8 md:flex md:flex-row">
      <div className="mx-auto max-md:hidden md:mt-3 md:w-[30%] md:min-w-48">
        <div className="max-md:block! group sticky top-3 z-10 max-h-[calc(100dvh-1.5rem)] overflow-y-auto">
