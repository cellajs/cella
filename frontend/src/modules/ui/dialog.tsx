...
>(({ className, disabled, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    className={cn(
      disabled
        ? ''
        : 'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-125 sm:grid place-items-center overflow-y-auto bg-muted/20 backdrop-blur-xs',
      className,
    )}
    {...props}
  />
...
>(({ className, children, container, hideClose, id, ...props }, ref) => {
  const renderContent = () => (
    <DialogPrimitive.Content
      ...
      ref={ref}
      className={cn(
        'overflow-x-clip overflow-y-clip bg-background mt-4 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 relative z-126 grid w-[95vw] gap-4 p-4 shadow-lg duration-200 rounded-lg mx-auto',
        className,
      )}
      {...props}
    >
