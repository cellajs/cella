...
>(({ className, disabled, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    className={cn(
      disabled
        ? ''
        : 'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 fixed inset-0 z-118 sm:grid place-items-center overflow-y-auto bg-background/40 backdrop-blur-xs',
      className,
    )}
    {...props}
  />
...
>(({ className, children, container, hideClose, id, ...props }, ref) => {
  const renderContent = () => (
    <DialogPrimitive.Content
      ...
      ref={ref}
      className={cn(
        'overflow-x-clip overflow-y-clip bg-background mt-4 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 relative z-119 grid w-[95vw] gap-4 border p-4 shadow-lg duration-200 rounded-lg mx-auto',
        className,
      )}
      {...props}
    >
      {children}
      {!hideClose && (
        <DialogPrimitive.Close className="ring-offset-background focus-visible:ring-ring data-[state=open]:bg-accent data-[state=open]:text-muted-foreground absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none">
