...
export const AboutSection = ({ title, text, sectionId, children, alternate = false }: AboutSectionProps) => {
  const { t } = useTranslation();

  const backgroundClass = alternate ? 'bg-accent/40 dark:bg-transparent' : '';

  return (
    <section id={sectionId} className={`container overflow-hidden max-w-none py-8 px-4 md:py-12 lg:py-24 ${backgroundClass}`}>
