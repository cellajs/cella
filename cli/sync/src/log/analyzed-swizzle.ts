...
export function analyzedSwizzleLine(analyzedFile: FileAnalysis): string {
  ...
  const parts: string[] = [filePath, status, swizzleEvent, lastSwizzledAt, statusTag].filter(Boolean);
...
export function logAnalyzedSwizzleLine(analyzedFile: FileAnalysis, line: string): void {
  ...
  const swizzledEqual =
    config.log.analyzedSwizzle.swizzled ===
    (analyzedFile.swizzle?.existingMetadata?.swizzled || analyzedFile.swizzle?.newMetadata?.swizzled);

  // Determine if the line should be logged based on all conditions
  const shouldLog = [shouldLogAnalyzedSwizzleModule(), !swizzledConfigured || swizzledEqual].every(Boolean);
